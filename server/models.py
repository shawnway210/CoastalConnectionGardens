...
product_price_association = db.Table(
    'product_price_association',
...
class Product(db.Model, SerializerMixin):
    ...
    # price_id = db.Column(db.Integer, db.ForeignKey('prices.id', name='fk_product_prices'), nullable = True)
    # cost_of_inventory_id = db.Column(db.Integer, db.ForeignKey('costOfInventory.id'))

    price = db.relationship('Price', secondary=product_price_association, backref='products')

    # inventory = db.relationship('Inventory', back_populates = 'product', uselist = False)
    # cost_of_inventory = db.relationship('CostOfInventory', back_populates='product', uselist=False)
    
    serialize_rules = ('-price.product',)
...
class Price(db.Model, SerializerMixin):
    ...
    serialize_rules = ('-product.prices',)
